...
class MedicalImageDataset(Dataset):
    def __init__(self, data, labels, model_name, transform=None, augmentation=None, mean=None, std=None):
        self.data = data
        self.labels = labels
    ...
    def _choose_transformation(self, modality):
        ...
        if self.model_name == "ResNet":
            ...
            elif self.augmentation == "albumentations":
                return v2.Compose([
                    cross_modality_transformer.transform,
            ...
            elif self.augmentation == "RandAugment":
                return v2.Compose([
                    cross_modality_transformer.transform,
                    v2.ToTensor(),
                    v2.RandAugment(),
...
def define_transformations(model, x_set, mean, std):
...
def run_experiment(runs, model, anatomy, augmentation, dataset_size=1.0):
    ...
    for runNr in range(runs):
        ...
        # Set current augmentation to mix with cross_modality
        ...
        train_transform = define_transformations(model, "train", mean, std)
        val_test_transform = define_transformations(model, "val_test", mean, std)
    ...
    file_path = 'average_roc_curve_' + model.__class__.__name__ + '_' + augmentation + '_cross_modality.png'
...
runs = 5
enable_wandb=False
with open("classification_performance_mixed.txt", "w+", encoding="utf-8") as result_file:
    
    
    result_file.write("ResNet \r\n\r\n")

    result_file.write("imgaug + cross_modality \r\n")
    ...
    result_file.write("albumentations + cross_modality \r\n")
    ...
    result_file.write("v2 + cross_modality \r\n")
    ...
    result_file.write("RandAugment + cross_modality \r\n")
